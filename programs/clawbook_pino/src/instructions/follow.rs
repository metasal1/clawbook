...
pub fn follow(
    ...
    let mut follow_account_state = FollowAccount::from_account_info_mut(follow_account)?;
    let follower_profile_state = Profile::from_account_info_mut(follower_profile)?;
    let mut following_profile_state = Profile::from_account_info_mut(following_profile)?;
    follow_account_state.follower = *authority.key () ;
    follow_account_state.following = following_profile_state.authority ;
    follow_account_state.created_at = Clock::get () ?.unix_timestamp ;
